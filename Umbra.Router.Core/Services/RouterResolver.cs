    ...
    where T : class, IRoutePage
{
    ...
    private RouterResult ResolveIRoutePage(NavigationDefinition definition, NavigationContext context)
    {
        ...
        _ = Task.Factory.StartNew(async () =>
            {
                try
                {
                    await vm.InitializeAsync(context).ConfigureAwait(false);
                }
                catch (Exception ex)
                {
        ...
        return new RouterResult(definition.View, vm, context, definition.Title);
    }
}
...
public class RouterResult
{
    ...
    public RouterResult(Type view, IRoutePage viewModel, NavigationContext context, string? titleTemplate)
    {
        View = view;
        ViewModel = viewModel;
        Context = context;
        TitleTemplate = titleTemplate ?? "{0}";
    }
