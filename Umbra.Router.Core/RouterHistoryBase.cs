    ...
    where TView : class
{
    ...
    private RouteSnapshot? CurrentRouter;
    ...
    public TViewModelBase? Go(int offset = 0)
    {
        ...
        var routerResult = ResolveViewModelAsync(context).Result;
    ...
    {
        return NavigateAsync(keys, body, title).Result;
    }

    public TViewModelBase? Navigate(string url, object? body = null, string title = "")
    {
        return NavigateAsync(url, body, title).Result;
    }

    public Task<TViewModelBase?> NavigateAsync(string[] keys, object? body = null, string title = "")
    {
        return NavigateAsync(new RouteSnapshot(keys, body), title);
    }

    public async Task<TViewModelBase?> NavigateAsync(string url, object? body = null, string title = "")
    {
        return await NavigateAsync(new RouteSnapshot(url, body), title);
    }


    public async Task<TViewModelBase?> NavigateAsync(RouteSnapshot snapshot, string title = "")
    {
        var routerResult = await ResolveViewModelAsync(snapshot);
        ...
        TitleChanged?.Invoke(string.Format(routerResult.TitleTemplate, title));

        Push(snapshot, title);
        return routerResult.ViewModel as TViewModelBase;
    }

    public void Push(RouteSnapshot item, string title = "")
    {
        if (HasNext)
        {
