...
public class RouterHistory<TViewModelBase> : RouterBase<TViewModelBase> where TViewModelBase : class, IRoutePage
{
    ...
    public RouterHistory(IServiceProvider serviceProvider, RouterConfig<TViewModelBase> config) : base(serviceProvider, config)
    {
    ...
    private void Push(NavigationContext item, string title = "")
    {
        if (HasNext)
        {
